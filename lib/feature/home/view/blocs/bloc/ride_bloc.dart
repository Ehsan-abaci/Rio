...
class RideBloc extends Bloc<RideEvent, RideState> {
  ...
  RideBloc(
    ...
    on<ReservedEvent>((event, emit) {
      ...
      emit(RideReserved(rideDetail: rideHistoryModel));
    });

    on<StartRidingEvent>((event, emit) async {
      emit(RideInProgress(rideDetail: rideHistoryModel));
      increaseAmount = 500;
      currentState = RideInProgress(rideDetail: rideHistoryModel);
      add(IncreaseAmount());
    });
    on<PausedEvent>((event, emit) async {
      emit(RidePaused(rideDetail: rideHistoryModel));
      increaseAmount = 100;
